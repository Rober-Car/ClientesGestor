 ...
 * FotoClienteCache
 * ----------------
 * Lectura AUTENTICADA de la foto de perfil del cliente (ADMIN) mediante Firebase
 * Storage SDK con caché local determinista. Es el MISMO patrón ya validado en
 * `:appCliente` para que el Admin no dependa de GET HTTP anónimo a la URL.
 *
 * Reglas:
 *  - La foto remota vive en `clientes/{clienteId}/foto.jpg` y la ficha guarda la
 *    URL con `?rev=` (revisión/cache-busting).
...
class FotoClienteCache @Inject constructor(
    ...
    suspend fun obtener(clienteId: Int, foto: String?): File? {
        ...
        return try {
            storage.reference.child(FotoClienteStorage.rutaCliente(clienteId))
                .getFile(temporal)
                .esperar()
